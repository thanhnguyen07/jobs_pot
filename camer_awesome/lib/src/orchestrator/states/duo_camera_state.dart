...
class DuoFilterModel {
  DuoFilterModel(this.captureRequest, this.filter);
...
typedef OnDuoPhotoCallback = Function(CaptureRequest request);

typedef OnDuoPhotoFailedCallback = Function(Exception exception);

/// When Camera is in Image mode
class DuoCameraState extends CameraState {
  DuoCameraState({
  ...
  factory DuoCameraState.from(CameraContext orchestrator) => DuoCameraState(
  ...
  @override
  CaptureMode get captureMode => CaptureMode.duo;
  ...
  Future<CaptureRequest> takePhoto({
    OnDuoPhotoCallback? onPhoto,
    OnDuoPhotoFailedCallback? onPhotoFailed,
  }) async {
  ...
  @override
  Future<void> setState(CaptureMode captureMode) async {
    if (captureMode == CaptureMode.duo) {
      return;
    } else {
      await cameraContext.setSensorConfig(SensorConfig.single(
